const { getData, createOrUpdateData,
    findById,
    findByEmail } = require("./services")
...
module.exports = {
    ...
    async createUser(req, res) {
        ...
        if (!name || !email || !password) return res.status(400).send({ message: "Não deve existir campos em branco" })
        try {
            if (name) {
    ...
    async login(req, res) {
        try {
            ...
            if (!email || !password) return res.status(400).send({ message: "Não deve existir campos em branco" })
            const user = findByEmail(email)
            if (!user || user.password !== password) return res.status(404).send('Credenciais inválidas')
